/**
 * `ail explain <code>` — the reasoning behind a design rule.
 *
...
export const explainCommand: Command = {
  ...
  run({ args }) {
    ...
    if (!code) {
      ...
      info('');
      info(dim('Run "ail explain AIL2503" for the reasoning behind one of them.'));
      return EXIT_OK;
    }

    const entry = CATALOGUE[code];
    if (!entry) {
      error(`no long-form explanation for ${code}. Run "ail explain" to see the codes that have one.`);
      return EXIT_USAGE;
    }
